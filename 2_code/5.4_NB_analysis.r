...
NB_analysis_parallel <- function(df_study, my_combis, my_timer){
  ...
  nb_model <- list()
  nb_accuracy <- list()
  nb_accuracy_dead <- list()
  nb_accuracy_alive <- list()
  nb_roc <- list()
  nb_auc <- list()
  nb_aucpr <- list()
  nb_mcc <- list()
  ...
  for (k in my_combis){
    ...
    # model name
    n_model = n_model + 1
    name <- paste('nb_model_', n_model, sep = '')
    ...
    # fit the model
    tic(name)
    nb_model[[name]] <- train(as.factor(dead) ~ ., 
                              data = train, 
                              method = 'naive_bayes',
                              trControl = fitControl) # train control
    my_timer <- rbind(my_timer, toc())
    # nb_model[[name]] <- naive_bayes(as.factor(dead) ~ ., data = train, userkernel = T) # package naivebayes
    print(paste('Model ', name, ' run succesfully.', sep = ''))
    
    # make a prediction 
    pred <- predict(nb_model[[name]], newdata = test, type = 'prob')
    ...
    nb_accuracy[[name]] <- 1 - pred_errors
    ...
    nb_accuracy_alive[[name]] <- right_alive / len_alive
    nb_accuracy_dead[[name]] <- right_dead / len_dead
  }
  ...
  results <- list(nb_model, nb_accuracy, nb_accuracy_dead, nb_accuracy_alive, nb_roc, nb_auc, nb_aucpr, 
                  nb_mcc, nb_kappa, nb_threshold, my_timer)  
  return(results)
}


NB_analysis <- function(df_study, my_combis, my_timer){
...
NB_analysis_timer <- function(df_study, my_combis, my_timer){
  # 
  # # activate timer
  # tic("NB")
  # 
  ...
  for (k in my_combis){
    ...
    # fit the model
    tic(paste('NB_', length(k), '_', nrow(df), sep = ''))
