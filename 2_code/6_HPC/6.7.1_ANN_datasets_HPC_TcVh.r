...
thinning = 'control'  # control, below, above
...
if(var_size %in% c('easy', 'medium', 'hard', 'extreme')){  # in that cases, the first record of each plot is not included
  df_original <- read.csv(paste('/home/aitorvazquez/PhD/Vitality/3_final/data/4_datasets/df_', thinning, '_time.csv', sep = ''))
} else {
  df_original <- read.csv(paste('/home/aitorvazquez/PhD/Vitality/3_final/data/4_datasets/df_', thinning, '.csv', sep = ''))
}
...
for (k in my_combis){
  ...
  # export data
  write.csv(train, paste('3_final/data/5_analysis/ann/input/T', thinning, '_V', var_size, '/train/', name, '.csv', sep = ''), row.names = FALSE)
  write.csv(test, paste('3_final/data/5_analysis/ann/input/T', thinning, '_V', var_size, '/test/', name, '.csv', sep = ''), row.names = FALSE)
}
...
# save my_timer
write.csv(my_timer, paste('3_final/data/5_analysis/ann/timer/T', thinning, '_V', var_size, '/ANN_1_timer.csv', sep = ''), row.names = FALSE)
