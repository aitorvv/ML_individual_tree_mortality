...
thinning = c('control', 'above')  # control, below, above
var_size = 'hard'  # easy, medium, hard, extreme
...
if(var_size %in% c('easy', 'medium', 'hard', 'extreme')){  # in that cases, the first record of each plot is not included
  df_original_1 <- read.csv(paste('/home/aitorvazquez/PhD/Vitality/3_final/data/4_datasets/df_', thinning[1], '_time.csv', sep = ''))
  df_original_2 <- read.csv(paste('/home/aitorvazquez/PhD/Vitality/3_final/data/4_datasets/df_', thinning[2], '_time.csv', sep = ''))
  df_original <- rbind(df_original_1, df_original_2)
  rm(df_original_1, df_original_2)
} else {
  df_original_1 <- read.csv(paste('/home/aitorvazquez/PhD/Vitality/3_final/data/4_datasets/df_', thinning[1], '.csv', sep = ''))
  df_original_2 <- read.csv(paste('/home/aitorvazquez/PhD/Vitality/3_final/data/4_datasets/df_', thinning[2], '.csv', sep = ''))
  df_original <- rbind(df_original_1, df_original_2)
  rm(df_original_1, df_original_2)
}
...
# tmp save
save.image(file = paste('3_final/data/5_analysis/T', thinning[1], thinning[2], '_V', var_size, '/tmp_metrics.RData', sep = ''))
...
save(lr_model, dt_model, rf_model, nb_model, svm_model, knn_model, file = 
       paste('3_final/data/5_analysis/T', thinning[1], thinning[2], '_V', var_size, '/models.RData', sep = ''))
...
save.image(file =  paste('3_final/data/5_analysis/T', thinning[1], thinning[2], '_V', var_size, '/metrics.RData', sep = ''))
