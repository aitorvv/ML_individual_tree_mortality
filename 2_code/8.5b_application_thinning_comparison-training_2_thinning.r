#------------------------------------------------------------------------------------------#
####         Application on thinning (training using 2 thinning) - comparison           ####
#                                                                                          #
...
#### Load general information: control + above thinning ####

# load case metrics
load('1_data/1_processed/7_applications/case_metrics_controlabove.RData') #***

# load all metrics
load('1_data/1_processed/6_final_results/best_models-training_2_thinning.RData')  #***

# remove functions
rm(find_best_model, graph_best_model, metric_graph, time_graph, all_timers)


#### Select metrics from original cases ####

# select metrics
original_metrics <- all_cases_best_model_compilation[all_cases_best_model_compilation$case %in% 'Tcontrolabove_Vhard', ]  #***
...
metrics$data_used <- ifelse(metrics$data_used == 'Tcontrol_Vhard', 'control', 
                            ifelse(metrics$data_used == 'Tbelow_Vhard', 'below', 
                                   ifelse(metrics$data_used == 'Tabove_Vhard', 'above', 'control + above'))) #***
desired_order <- c("control + above", "control", "above", "below") #***
...
df_graph <- metrics
graph_title <- 'application-training_2_thinning/Tcontrolabove_to_others' #***
...
pl_control <- pointline_by_cs(df_graph, x_axis = factor(metrics$data_used), y_axis, fill, g_title, g_x, g_y, g_legend, color_groups, 
                              graph_title = 'application-training_2_thinning/pointline-controlabove_to_others', 
...
#### Load general information: control + below thinning ####

# load case metrics
load('1_data/1_processed/7_applications/case_metrics_controlbelow.RData') #***
case_metrics$data_used <- ifelse(case_metrics$data_used %in% 'control_Vhard', 'Tcontrol_Vhard', case_metrics$data_used)

#### Select metrics from original cases ####

# select metrics
original_metrics <- all_cases_best_model_compilation[all_cases_best_model_compilation$case %in% 'Tcontrolbelow_Vhard', ] #***
original_metrics <- original_metrics[original_metrics$Metrics %in% 'mcc', ]
original_metrics <- original_metrics[!original_metrics$names_methods %in% 'ANN', ]
...
metrics$data_used <- ifelse(metrics$data_used == 'Tcontrol_Vhard', 'control', 
                            ifelse(metrics$data_used == 'Tbelow_Vhard', 'below', 
                                   ifelse(metrics$data_used == 'Tabove_Vhard', 'above', 'control + below'))) #***
desired_order <- c("control + below", "control", "above", "below") #***
...
metrics$facet <- metrics$data_used
df_graph <- metrics
graph_title <- 'application-training_2_thinning/Tcontrolbelow_to_others' #***

# graph results
bar_by_cs(df_graph, x_axis, y_axis, fill, g_title, g_x, g_y, g_legend, color_groups, graph_title)
pl_above <- pointline_by_cs(df_graph, x_axis = factor(metrics$data_used), y_axis, fill, g_title, g_x, g_y, g_legend, color_groups, 
                            graph_title = 'application-training_2_thinning/pointline-controlbelow_to_others', 
                            min_shadow = min_shadow, max_shadow = max_shadow, y_min = 0, y_max = 0.8)
...
#### Load general information: above + below thinning ####

# load case metrics
load('1_data/1_processed/7_applications/case_metrics_abovebelow.RData') #***
case_metrics$data_used <- ifelse(case_metrics$data_used %in% 'control_Vhard', 'Tcontrol_Vhard', case_metrics$data_used)

#### Select metrics from original cases ####

# select metrics
original_metrics <- all_cases_best_model_compilation[all_cases_best_model_compilation$case %in% 'Tabovebelow_Vhard', ] #***
original_metrics <- original_metrics[original_metrics$Metrics %in% 'mcc', ]
original_metrics <- original_metrics[!original_metrics$names_methods %in% 'ANN', ]
...
metrics$data_used <- ifelse(metrics$data_used == 'Tcontrol_Vhard', 'control', 
                            ifelse(metrics$data_used == 'Tbelow_Vhard', 'below', 
                                   ifelse(metrics$data_used == 'Tabove_Vhard', 'above', 'above + below'))) #***
desired_order <- c("above + below", "control", "above", "below") #***
...
metrics$facet <- metrics$data_used
df_graph <- metrics
graph_title <- 'application-training_2_thinning/Tabovebelow_to_others' #***

# graph results
bar_by_cs(df_graph, x_axis, y_axis, fill, g_title, g_x, g_y, g_legend, color_groups, graph_title)
pl_below <- pointline_by_cs(df_graph, x_axis = factor(metrics$data_used), y_axis, fill, g_title, g_x, g_y, g_legend, color_groups, 
                            graph_title = 'application-training_2_thinning/pointline-abovebelow_to_others', 
                            min_shadow = min_shadow, max_shadow = max_shadow, y_min = 0, y_max = 0.8)
...
metrics$model_used <- ifelse(metrics$model_used %in% 'Tcontrolbelow_Vhard', 'control + below',
                             ifelse(metrics$model_used %in% 'Tabovebelow_Vhard', 'above + below', 'control + above'))
metrics$data_used <- ifelse(metrics$data_used %in% 'Tcontrol_Vhard', 'control',
                            ifelse(metrics$data_used %in% 'Tbelow_Vhard', 'below', 
                                   ifelse(metrics$data_used %in% 'Tabove_Vhard', 'above', 
                                          ifelse(metrics$model_used %in% 'Tcontrolbelow_Vhard', 'control + below',
                                                 ifelse(metrics$model_used %in% 'Tabovebelow_Vhard', 'above + below', 
                                                        'control + above')))))
metrics$description <- paste(metrics$model_used, ' model x ', metrics$data_used, ' data', sep = '')

# reorder the levels of the 'data' variable based on your preferred order
desired_order <- c("control + below model x control data", "control + below model x above data" , "control + below model x below data",
                   "control + above model x control data", "control + above model x above data", "control + above model x below data",
                   "above + below model x control data", "above + below model x above data", "above + below model x below data"
) #***
...
df_graph <- metrics
graph_title <- 'application-training_2_thinning/all_cases'
...
ggsave("2_scripts/4_figures/9.4_best_model_metrics/application-training_2_thinning/pointline_all_cases.png", units = 'mm', dpi = 300, width = 600, height = 300)
...
ggsave("2_scripts/4_figures/9.4_best_model_metrics/application-training_2_thinning/pointline_all_cases_dashed.png", units = 'mm', dpi = 300, width = 600, height = 300)
...
ggsave("2_scripts/4_figures/9.4_best_model_metrics/application-training_2_thinning/pointline_all_cases_point.png", units = 'mm', dpi = 300, width = 600, height = 300)
