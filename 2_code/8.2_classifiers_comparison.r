...
# split case study in 3 variables
df$data <- substring(df$case, 0, 13)
df$vars <- str_sub(df$case, -1, -1)
...
#                                                                    ifelse(df$data == 'L6', '6to7', 
#                                                                           ifelse(df$data == 'L7', '7to9', ''))))))))))
df$data <- ifelse(df$data == 'Tcontrolabove', 'control + above', 
                  ifelse(df$data == 'Tcontrolbelow', 'control + below', 
                         ifelse(df$data == 'Tabovebelow_V', 'above + below', df$data)))
...
#### Analysis of dataset thinning: training with 2 thinning groups ####

# all data
df_cs <- df[df$case_group == 'thinning_2', ]


#### Analysis of dataset thinning: graph by thinning groups (training with 2 thinning groups) ####

# reorder the levels of the 'data' variable based on your preferred order
desired_order <- c("control + above", "control + below", "above + below") 
...
y_axis <- df_cs$best_acc
fill <- df_cs$names_methods
g_title <- 'Best model per classifier performance among thinning regimes (training with 2 thinning groups)'
g_x <- 'Algorithm'
g_y <- 'MCC'
g_legend <- 'Algorithm'
color_groups <- color_methods
df_cs$facet <- df_cs$data
df_graph <- df_cs
graph_title <- 'thinning_2/classifiers_per_thinning'

# graph results
bar_by_cs(df_graph, x_axis, y_axis, fill, g_title, g_x, g_y, g_legend, color_groups, graph_title)


#### Analysis of dataset thinning: graph by classifiers (training with 2 thinning groups) ####
...
df_cs$facet <- df_cs$names_methods
df_graph <- df_cs
graph_title <- 'thinning_2/classifiers_per_thinning_split'

# graph results
bar_by_classifiers(df_graph, x_axis, y_axis, fill, g_title, g_x, g_y, g_legend, color_groups, graph_title)


# #### Analysis of dataset thinning x variables (training with 2 thinning groups) ####
# 
# # all data
# df_cs <- df[df$case_group == 'thinning_2', ]
# 
# 
# #### Analysis of dataset thinning: graph by var groups (training with 2 thinning groups) ####
# 
# # reorder the levels of the 'data' variable based on your preferred order
# desired_order <- c("control - I", "control - II", "control - III", "control - IV",
#                    "below - I", "below - II", "below - III", "below - IV",
...
# df_cs$facet <- df_cs$case
# df_graph <- df_cs
# graph_title <- 'thinning_2-vars/classifiers_per_thinning-vars'
# 
# # graph results
# bar_by_cs(df_graph, x_axis, y_axis, fill, g_title, g_x, g_y, g_legend, color_groups, graph_title)
...
y_axis <- df_cs$best_acc
fill <- df_cs$case
g_title <- 'Best model per classifier performance among thinning regimes and number of variables (training with 2 thinning groups)'
g_x <- 'Algorithm'
g_y <- 'MCC'
g_legend <- 'Case study'
color_groups <- color_methods_cases
df_cs$facet <- df_cs$names_methods
df_graph <- df_cs
graph_title <- 'thinning_2-vars/classifiers_per_thinning-vars_split'
