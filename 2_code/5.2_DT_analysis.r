...
DT_analysis_timer <- function(df_study, my_combis, my_timer){
  # 
  # # activate timer
  # tic("DT")
  # 
  ...
  for (k in my_combis){
    ...
    # fit the model
    tic(paste('DT_', length(k), '_', nrow(df), sep = ''))
    # dt_model[[name]] <- train(as.factor(dead) ~ ., data = train, method = 'rpart', trControl = fitControl)
    train(as.factor(dead) ~ ., data = train, method = 'rpart', trControl = fitControl)
    my_timer <- rbind(my_timer, toc())
  }
  #   print(paste('Model ', name, ' run succesfully.', sep = ''))
  #   
