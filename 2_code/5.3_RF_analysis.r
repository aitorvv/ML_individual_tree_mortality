...
RF_analysis_timer <- function(df_study, my_combis, my_timer){
  # 
  # # activate timer
  # tic("RF")
  # 
  ...
  for (k in my_combis){
    ...
    # fit the model
    tic(paste('RF_', length(k), '_', nrow(df), sep = ''))
    # rf_model[[name]] <- caret::train(dead ~ ., # "dead" is a function of the variables we decided to include
    caret::train(dead ~ ., # "dead" is a function of the variables we decided to include
                                     data = train, # Use the train data frame as the training data
                                     method = 'rf', # Use the 'random forest' algorithm
                                     trControl = fitControl) # train control
    my_timer <- rbind(my_timer, toc())
